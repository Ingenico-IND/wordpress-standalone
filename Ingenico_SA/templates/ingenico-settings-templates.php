...
class Ingenico_Templates
{
	function adminOptions()
    {
        echo
            '<div class="wrap">
                <h2><b>Ingenico Payment Gateway</b></h2>';

                ?><a href="#" target="_blank"><img style="margin-top: 10px;" src="<?php echo plugins_url('images/logo.png' , __FILE__ );?>"/></a><?php


                echo'<form action="options.php" method="POST">';

                    settings_fields('ingenico_fields');
                    do_settings_sections('ingenico_sections');
                    submit_button();
                    settings_errors();
    ...
    {
    	add_settings_section('ingenico_fields', 'Edit Settings', array($this, 'displayHeader'), 'ingenico_sections');
        ...
        foreach ($settings as $settingField => $settingName)
        {
            ...
            register_setting('ingenico_fields', $settingField);
        }
    }

    function displayHeader()
    {
        $header = '<p>Ingenico is an online payment gateway</p>';
    ...
    {
        $default = get_option('description_field',"Ingenico Payment Gateway");
    ...
    function displayPaymentMode()
    {
        ...
        <label for ="payment_mode">If Bank selection is at Ingenico ePayments India Pvt. Ltd. (a Worldline brand) end then select all, if bank selection at Merchant end then pass appropriate mode respective to selected option</label>
        EOT;
    ...
    function displayEmbedPaymentGatewayOnPage()
    {
        ...
        $selected_true = ($default == '#ingenico_payment_form') ? 'selected' : '' ;
        ...
        <select style="width:350px;" name="embed_payment_gateway_on_page_field" id="embed_payment_gateway_on_page" value="{$default}"/>
            <option value="#ingenico_payment_form" {$selected_true}>Enable</option>
