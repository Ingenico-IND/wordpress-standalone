...
if ( ! defined( 'ABSPATH' ) )
    exit;
include_once(dirname(__FILE__).'/database_ingenico.php');
require_once __DIR__.'/includes/Ingenico-settings.php';
add_action( 'wp_enqueue_scripts', 'enqueue_style_script' );

function enqueue_style_script(){
    wp_enqueue_style( "Ingenico_order_style", plugins_url( '/assets/In_style.css', __FILE__ ));
    wp_enqueue_script( "Ingenico_order_script", plugins_url( '/assets/In_script.js', __FILE__ ));
}
...
add_action('plugins_loaded', 'wordpressIngenicoInit', 0); 

function wordpressIngenicoInit()
{
    if ( !defined('PLUGIN_BASE_NAME' ) )
    {
        define('INGENICO_BASE_NAME', plugin_basename(__FILE__));
    }
    class WP_Ingenico
    {
        public function __construct()
        {
            ...
            $settings = new Ingenico_Settings();
            $this->enable                           = get_option('enabled_field');
            $this->title                            = get_option('title_field');
            $this->description                      = get_option('description_field');
            $this->ingenico_merchant_code           = get_option('merchant_code_field');
            $this->ingenico_SALT                    = get_option('salt_field');
            $this->payment_type                     = get_option('payment_type_field');
            $this->currency_type                    = get_option('currency_field');
            $this->ingenico_merchant_scheme_code    = get_option('merchant_scheme_code_field');
            $this->ingenico_success_msg             = get_option('success_message_field');
            $this->ingenico_decline_msg             = get_option('decline_message_field');
            ...
            $this->ingenico_payment_mode            = get_option('payment_mode_field');
            ...
            add_shortcode('INGENICO', array($this, 'checkout'));
            add_filter('plugin_action_links_' . INGENICO_BASE_NAME, array($this, 'IngenicoSettingLinks'));
            add_action('wp_ajax_ingenico_response_handle', array($this, 'CheckIngenicoResponse'),10);
            add_action('wp_ajax_nopriv_ingenico_response_handle', array($this, 'CheckIngenicoResponse'),10);
            add_action('wp_ajax_save_to_db', array($this, 'Ingenico_Initial_Response'),10);
            add_action('wp_ajax_nopriv_save_to_db', array($this, 'Ingenico_Initial_Response'),10);
            add_action('init',array($this,'register_session'));
        }

        function IngenicoSettingLinks($links)
        {
            $SettingLinks = array(
                            'settings' => '<a href="'. esc_url(admin_url('admin.php?page=ingenico')) .'">Settings</a>'
                        );
        ...
        function checkout(){
            $html = $this->generateIngenicoOrderForm();
            return $html;
        }
        ...
        function generateIngenicoOrderForm()
        {
            $pageID = get_the_ID();
            ...
            if ( isset ( $this->ingenico_merchant_code ) && isset ( $this->ingenico_SALT ) && isset ( $this->ingenico_merchant_scheme_code ) && $amount != null)
            {
                ...
                $mrctCode                           = $this->ingenico_merchant_code;
                $mobNo                              = '';
                $email                              = '';
                $name                               = '';
                $returnUrl                          = '';
                $SALT                               = $this->ingenico_SALT;
                $accNo                              = '';
                $debitStartDate                     = '';
                $debitEndDate                       = '';
                $maxAmount                          = '';
                ...
                if( !$payment_order_mode_raw || $payment_order_mode_raw == "all" ){
                    ...
                    $pmo['paymentModeOrder'] =  [   $payment_order_mode[0],
                ...
                $data=array(
                    ...
                    "data"=>array(
                        ...
                        $returnUrl,                                 //12
                        $name,                                      //13
                        $this->ingenico_merchant_scheme_code,       //14
        ...
        function Ingenico_Initial_Response(){
        ...
        function CheckIngenicoResponse()
        {
            ...
            if (isset($_REQUEST['msg'])){
                ...
                if (!empty($status_before_dv)) {
                    ...
                    $wpdb->update( 
                        $table_name, 
                        array(
                            'ingenico_identifier'   =>  $response1[5],
                            'response'              =>  $str,
                            'status'                =>  $response1[1],   
                    ...
                    if ($hashed_string_token == $hashstring) {
                        if ($status_before_dv == '300') {
                            ...
                            echo $this->ingenico_success_msg."<br>";   
                                $wpdb->update( 
                                    $table_name, 
                                    array(
                                        'status'    =>  'success'   
                                ...
                                echo $this->ingenico_decline_msg."<br>" ;
                                $wpdb->update( 
                                    $table_name, 
                                    array(
                                        'status'    =>  'failed'   
                        ...
                        echo $this->ingenico_decline_msg."<br>".'Transaction Error Message from Payment Gateway: Hash Validation Failed<br>';
                        $wpdb->update( 
                            $table_name, 
                            array(
    ...
    return new WP_Ingenico();
}

function my_S2S_route() {
    register_rest_route( 'ingenico', '/s2sverification', array(
            'methods' => 'POST',
...
function callback_S2S() {
    ...
    $array_without_hash = $response1;
    $string_without_hash = implode( "|", $array_without_hash);
    $salt_token = $string_without_hash. '|'.$paynimo_class->ingenico_SALT;
    $hashed_string_token = hash('sha512', $salt_token);
    $dir_path = plugin_dir_path(__FILE__);
    $directoryname = 'logs';
    $dir_name = $dir_path . 'logs/';                     
    if ($hashstring == $hashed_string_token) {
        if ($status == '300') {
            $file_name = 'Ingenico_S2S_logs'.date("Y-m-d").'.log';
            if ( !file_exists($file_name ) ) {
                $myfile = fopen( $dir_name . $file_name, "a");
                $txt =  "\r\n"."Response_S2S:". $str;
                $write_file = fwrite($myfile, $txt);
